    ...
    <section className="section-white py-16 md:py-24 px-4 border-y border-border">
      <div className="max-w-7xl mx-auto">
        <header className="mb-10 md:mb-12 max-w-3xl">
          <p className="section-eyebrow mb-2">{t('presence.eyebrow')}</p>
          <h2 className="text-2xl md:text-3xl font-semibold text-charcoal mb-3">{t('presence.title')}</h2>
          <p className="text-muted text-sm md:text-base leading-relaxed mb-3">{t('presence.desc')}</p>
          <p className="text-sm text-[#666666] leading-relaxed">{t('presence.demoNote')}</p>
        </header>
        ...
        <div className="section-cream rounded-xl border border-border p-8 md:p-10 mb-10">
          ...
          <h3 className="text-xl md:text-2xl font-semibold text-charcoal mb-3">{t('presence.trustTitle')}</h3>
          <p className="text-sm md:text-base text-muted leading-relaxed max-w-3xl">{t('presence.trustDesc')}</p>
        </div>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-10 text-center sm:text-left">
          <LocationVisual />
          <div>
            <p className="text-xs section-eyebrow mb-1">{t('presence.locationEyebrow')}</p>
            <p className="text-lg md:text-xl font-semibold text-charcoal tracking-wide">{t('presence.location')}</p>
          </div>
        </div>
      </div>
    </section>
